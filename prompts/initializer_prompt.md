...
### 2.5 TASK: Dokploy Deployment Pack (ONLY if app_spec requests Dokploy)

If `app_spec.txt` includes:

- `<deployment><target>dokploy</target>` AND `<mode>docker_compose</mode>`

then you MUST create a Dokploy-ready deployment pack in the PROJECT ROOT:

**Required files:**
- `Dockerfile` (production build/run)
- `docker-compose.yml` (single app service unless spec explicitly requires more)
- `.env.example` (non-secret env vars; MUST include `PORT=3000`)
- `deploy.dokploy.json` (machine-readable deployment metadata)

**Port contract (CRITICAL):**
- The app MUST read runtime port from env var `PORT`
- Default to `3000` if not set
- Bind to `0.0.0.0:$PORT` (not localhost)
- In `docker-compose.yml`, expose the same port so Dokploy UI can route traffic to it

**deploy.dokploy.json format (minimum):**
```json
{
  "platform": "dokploy",
  "mode": "docker_compose",
